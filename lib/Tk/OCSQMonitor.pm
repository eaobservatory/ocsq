...
sub Populate {
    ...
    my $shiftmenu = $shiftmenubutton->Menu(
        -tearoff => 0,
        -menuitems => [
            map {
                my $type = $_;
    ...
    $Fr3->Label(-text => 'Shift type:',)->pack(-side => 'right');
    ...
    my $MsgBut = $w->Checkbutton(
        -variable => \$priv->{MORE_INFO},
        -text => 'Info messages...',
        -command => ['show_info', $w],
    );

    my $MsgTextFrame = $w->Frame();
    my $MsgText = $MsgTextFrame->Scrolled(
        'TextANSIColor',
        -scrollbars => 'w',
        -height => 16,
        -width => $args->{'-msgwidth'},
    ...
    $priv->{MORE_ERS} = 1;
    my $ErsBut = $w->Checkbutton(
        -variable => \$priv->{MORE_ERS},
        -text => 'Error messages...',
        -command => ['show_ers', $w],
    );

    my $ErsTextFrame = $w->Frame();
    my $ErsText = $ErsTextFrame->Scrolled(
        'TextANSIColor',
        -scrollbars => 'w',
        -height => 8,
        -width => $args->{'-msgwidth'},
...
sub _shutdown {
    ...
    if (exists $priv->{MONITOR}->{MONITOR_ID}) {
        if (exists $priv->{MONITOR}->{MONITOR_ID} && ! $priv->{QUEUE_CANCELLED}) {
            monitor($w->cget('-qtask'), "CANCEL",
                $priv->{MONITOR}->{MONITOR_ID});
        }
...
sub init_queue_monitor {
    ...
    monitor(
        ...
        'SHIFTTYPE',
        {
            ...
            -repmonloss => 1,
            -complete => sub {
                $w->write_text_messages('messages', "Monitor complete\n");
                $priv->{QUEUE_CANCELLED} = 1;
            },
...
sub cvtsub {
    ...
    if ($param eq 'STATUS') {
        ...
        if (defined $value && $value > 0) {
            ...
            elsif ($value == Queue::Constants::QSTATE__IDLETIMEOUT) {
                $sound = "queueidle.wav";
            }
            _play_sound($sound);
        }
    ...
    if ($param eq 'Queue') {
        ...
        if (defined $tie{MESSAGE}) {
            my @lines =
                (ref $tie{MESSAGE} ? @{$tie{MESSAGE}} : ($tie{MESSAGE}));
            _clean_array(\@lines);
            chomp(@lines);
...
sub update_listboxes {
    ...
    for my $line (@contents) {
        ...
        $ContentsBox->tag('bind', $dtag,
            '<ButtonRelease-1>' => sub {
                pset($w->cget('-qtask'), "INDEX", $index)});

        # and to the right mouse button
        $ContentsBox->tag('bind', $dtag,
            '<Button-2>' => [\&ContentsMenu, $w, $index, Ev('X'), Ev('Y')],);
...
sub respond_to_failure {
    ...
    if ($details->{REASON} eq 'MissingTarget'
        ...
        if ($details->{TELESCOPE} eq 'JCMT') {
            ...
            die "Should not happen. Telescope was "
                . $details->{TELESCOPE} . "\n";
        }
    }
    else {
...
sub create_fail_gui {
    ...
    if ($details->{CAL}) {
        ...
        $cat->pushstar(map {new Astro::Catalog::Item(coords => $_)} @planets,
            scuba_2cals());
    }
    else {
...
sub create_msbcomplete_tab {
    ...
    my $Reason = $w->Text(-height => 5, -width => 50)
...
sub msbcompletion {
    ...
    if ($$userid_gui ne $$userid) {
        _log_user_id('Tk::OCSQMonitor::msbcompletion - accept userid changed',
            $$userid, $$userid_gui);
