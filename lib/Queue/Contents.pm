...
We hit an entry that has a valid target

=item 2

Once we hit a calibration observation we continue
to propogate until we are no longer doing calibrations.
This allows us to propogate through a set of 6 scan maps.

=item 3

We hit the last observation in an MSB.
