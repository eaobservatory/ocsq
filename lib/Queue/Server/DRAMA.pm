...
sub set_msbcomplete_parameter {
    ...
    $details{MSBTID} = $details{MSB}->transid
        if (exists $details{MSB} && defined $details{MSB});
...
sub archive_pending_msbcomplete {
    ...
    if (keys %pending) {
        ...
        if ($@) {
            print "Log warning: Unable to write pending complete information to "
                . $self->_pending_msb_filename . ": $@";
        }
    }
    else {
...
sub SUSPENDMSB {
    ...
    if ($proj && $msbid && $label) {
        ...
        try {
            ...
            $Q->addmessage($status,
                "MSB for project $proj has been suspended at the current observation $msg"
            );
        }
        otherwise {
            # Error in suspend
        ...
        unless ($err_found) {
            if ($Q->queue->backend->qrunning) {
                ...
                if ((defined $last_active)
                        and ((time - $last_active) > $idle_time)) {
                    $Q->last_active(undef);
                    update_alert_param(Queue::Constants::QSTATE__IDLETIMEOUT,
                        $status);
                    $Q->addmessage($status,
                        "The queue has been idle for $idle_time seconds.");
                }
            }
        }
    }
...
User associated with this request [optional]

=item REASON

String describing any particular reason
...
sub MSBCOMPLETE {
    ...
    for my $donemsb (@completed) {
        ...
        if (! $donemsb->{compkey}) {
            ...
            $Q->addmessage($status,
                "Attempting to mark MSB with completion key " . $donemsb->{compkey}
                . " as complete but can no longer find it in the parameter system."
            );
            next;
        }
        ...
        $Q->addmessage($status,
            "Attempting to mark MSB '$msbtitle' with completion key " . $donemsb->{compkey}
            . " (transaction $msbtid) as complete [$martxt]");
        ...
        if ($mark > 0) {
            try {
                ...
                if ($Q->simdb) {
                    ...
                    # which always fails.
                    eval {
                        $msbserv->doneMSB($projectid, $msbid,
                            $donemsb->{userid},
            ...
            otherwise {
                ...
                $Q->addmessage($status,
                    "Error marking msb $msbid as rejected: $E");
            };
        }
        else {
            $Q->addmessage($status,
...
sub msbtidy {
    ...
    if ($msbid && $projectid) {
        ...
        if ($projectid eq 'SCUBA' || $projectid =~ /CAL$/) {
            ...
            $Q->addmessage($status,
                "Possibility of marking MSB for project $projectid as done [MSB=$msbid TID="
                . $msb->transid . "]");
